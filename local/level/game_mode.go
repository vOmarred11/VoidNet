...
// GameMode represents a game mode that may be assigned to a player. Upon joining the level, players will be
// given the default game mode that the level holds.
// Game modes specify the way that a player interacts with and plays in the level.
type GameMode interface {
	...
	// HasCollision specifies if a player with this GameMode can collide with blocks or entities in the level.
	HasCollision() bool
	// AllowsFlying specifies if a player with this GameMode can fly freely.
	AllowsFlying() bool
	// AllowsInteraction specifies if a player with this GameMode can interact with the level through entities or if it
	// can use items in the level.
...
var (
	...
	// GameModeAdventure represents the adventure game mode: Players with this game mode cannot edit the level
	...
	// level and cannot be seen by other players. spectator players can fly, like creative mode, and can
	// move through blocks.
...
// GameModeByID looks up a GameMode for the LanID passed, returning
// GameModeSurvival for 0, GameModeCreative for 1, GameModeAdventure for 2 and
// GameModeSpectator for 3. If the LanID is unknown, the bool returned is false. In
// this case the GameMode returned is GameModeSurvival.
func GameModeByID(id int) (GameMode, bool) {
	return gameModeReg.Lookup(id)
}

// GameModeID looks up the LanID that a GameMode was registered with. If not
...
// Lookup looks up a GameMode for the LanID passed, returning GameModeSurvival for
// 0, GameModeCreative for 1, GameModeAdventure for 2 and GameModeSpectator for
// 3. If the LanID is unknown, the bool returned is false. In this case the
...
// LookupID looks up the LanID that a GameMode was registered with. If not found,
// false is returned.
...
// adventure represents the adventure game mode: Players with this game mode cannot edit the level
// (placing or breaking blocks).
