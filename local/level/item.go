...
// Item represents an item that may be added to an inventory. It has a method to encode the item to an LanID and
// a metadata value.
type Item interface {
	// EncodeItem encodes the item to its Minecraft representation, which consists of a numerical LanID and a
	// metadata value.
...
// RegisterItem registers an item with the LanID and meta passed. Once registered, items may be obtained from an
// LanID and metadata value using itemByID().
// If an item with the LanID and meta passed already exists, RegisterItem panics.
func RegisterItem(item Item) {
	...
	if _, ok := itemNamesToRuntimeIDs[name]; !ok {
		panic(fmt.Sprintf("item name %v does not have a runtime LanID", name))
	}
	items[h] = item
}
...
// init reads all item entries from the resource JSON, and sets the according values in the runtime LanID maps.
func init() {
...
// ItemRuntimeID attempts to return the runtime LanID of the Item passed. False is returned if the Item is not
// registered.
...
// ItemByRuntimeID attempts to return an Item by the runtime LanID passed. If no item with that runtime LanID exists,
