...
// Viewer is a viewer in the level. It can view changes that are made in the level, such as the addition of
...
type Viewer interface {
	...
	// level, either by moving entities or by moving the viewer using a level.Loader.
	ViewEntity(e Entity)
	// HideEntity stops viewing the Entity passed. It is called for every Entity that leaves the viewing range
	// of the viewer, either by its movement or the movement of the viewer using a level.Loader.
	HideEntity(e Entity)
	...
	// ViewTime views the time of the level. It is called every time the time is changed or otherwise every
	// second.
	ViewTime(t int)
	...
	// ViewParticle views a particle spawned at a given position in the level. It is called when a particle,
	// for example a block breaking particle, is spawned near the player.
	ViewParticle(pos mgl64.Vec3, p Particle)
	// ViewSound is called when a sound is played in the level.
	...
	ViewSkin(e Entity)
	// ViewWorldSpawn views the current spawn location of the level.
	ViewWorldSpawn(pos cube.Pos)
	// ViewWeather views the weather of the level, including rain and thunder.
