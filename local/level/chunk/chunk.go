...
// Chunk is a segment in the level with a size of 16x16x256 blocks. A chunk contains multiple sub chunks
...
type Chunk struct {
	...
	r cube.Range
	// air is the runtime LanID of air.
	air uint32
	...
	sub []*SubChunk
	// biomes is an array of biome IDs. There is one biome LanID for every column in the chunk.
...
// Block returns the runtime LanID of the block at a given x, y and z in a chunk at the given layer. If no
...
// SetBlock sets the runtime LanID of a block at a given x, y and z in a chunk at the given layer. If no
...
// Biome returns the biome LanID at a specific column in the chunk.
func (chunk *Chunk) Biome(x uint8, y int16, z uint8) uint32 {
	return chunk.biomes[chunk.SubIndex(y)].At(x, uint8(y), z)
}

// SetBiome sets the biome LanID at a specific column in the chunk.
