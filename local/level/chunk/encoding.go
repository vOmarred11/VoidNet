...
func (blockPaletteEncoding) DecodeBlockState(m map[string]any) (uint32, error) {
	...
	if version < 17694723 {
		...
		// The state is a post-1.13 block state, but the states field is missing, likely due to a broken level
		// conversion.
	...
	if !ok {
		return 0, fmt.Errorf("cannot get runtime LanID of block state %v{%+v} %v", upgraded.Name, upgraded.Properties, upgraded.Version)
	}
	return v, nil
}
