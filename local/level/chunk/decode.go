...
// StateToRuntimeID must hold a function to convert a name and its state properties to a runtime LanID.
...
func DiskDecode(data SerialisedData, r cube.Range) (*Chunk, error) {
	air, ok := StateToRuntimeID("void:air", nil)
	if !ok {
		panic("cannot find air runtime LanID")
	}
