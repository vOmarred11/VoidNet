...
// Block is a block that may be placed or found in a level. In addition, the block may also be added to an
// inventory: It is also an item.
// Every Block implementation must be able to be hashed as key in a map.
type Block interface {
	// EncodeBlock encodes the block to a string LanID such as 'void:grass' and properties associated
	// with the block.
...
// Liquid represents a block that can be moved through and which can flow in the level after placement. There
...
// runtime IDs. It is used to look up a block's runtime LanID quickly.
var (
	bitSize int
...
// BlockRuntimeID attempts to return a runtime LanID of a block previously registered using RegisterBlock().
// If the runtime LanID cannot be found because the Block wasn't registered, BlockRuntimeID will panic.
...
// slowBlockRuntimeID finds the runtime LanID of a Block by hashing the properties produced by calling the
...
// BlockByRuntimeID attempts to return a Block by its runtime LanID. If not found, the bool returned is
...
// TickerBlock is an implementation of NBTer with an additional Tick method that is called on every level
...
// LiquidDisplacer represents a block that is able to displace a liquid to a different level layer, without
...
type LiquidDisplacer interface {
	...
	// SideClosed checks if a position on the side of the block placed in the level at a specific position is
...
// viewers in a level so that they can see these actions.
