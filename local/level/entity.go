...
// LanID and bounding box of an Entity.
...
type EntityHandle struct {
	...
	// TODO Handler? Handle level change here?
}
...
// EntityHandle may be added to a level by calling Tx.AddEntity().
...
// passed. The EntityHandle may be added to a level by calling Tx.AddEntity().
...
// A non-nil Entity is returned only if the entity's level matches the level of
...
func (e *EntityHandle) mustEntity(tx *Tx) Entity {
	...
	panic("can't load entity with Tx of different level")
}
...
// level, ExecWorld will block until the EntityHandle is added to a World and
...
// execWorld uses a sync.Cond to synchronise access to the handler's level. We
...
func (e *EntityHandle) execWorld(f func(tx *Tx, e Entity), weak bool) bool {
	...
	// e.unsetAndLockWorld(), where the entity's level is removed.
	...
	// transaction must await earlier transactions in the level. If one of those
...
// weakExec performs a "weak transaction". It adds a transaction to the level
...
// any goroutines waiting for a non-nil level are awoken.
func (e *EntityHandle) setAndUnlockWorld(w *World) {
	e.cond.L.Lock()
	defer e.cond.L.Unlock()

	if e.w != nil {
		panic("cannot add entity to new level before removing from old level")
	}
	e.w = w
	e.cond.Broadcast()
}
...
// Entity represents an Entity in the level, typically an object that may be moved around and can be
// interacted with by other entities.
// Viewers of a level may view an Entity when near it.
type Entity interface {
	io.Closer
	// H returns the EntityHandle that points to the entity.
	H() *EntityHandle
	// Position returns the current position of the Entity in the level.
	Position() mgl64.Vec3
