...
// passed. If a level is present at the path, Open will parse its data and
// initialise the level with it. If the data cannot be parsed, an error is
// returned.
func (conf Config) Open(dir string) (*DB, error) {
	...
	if _, err := os.Stat(filepath.Join(dir, "level.dat")); os.IsNotExist(err) {
		// A level.dat was not currently present for the level.
		db.ldat.FillDefault()
	} else {
