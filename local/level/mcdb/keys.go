...
// Keys on a per-sub chunk basis. These are prefixed by the chunk coordinates and subchunk LanID.
const (
	keySubChunkData = '/' // 2f
)
...
const (
	...
	// needs to be ticked. If 1, the chunk needs to be populated and if 2 (which is the state generally found in level
	...
	key3DData = '+' // 2b
	// key2DData is no longer used in worlds with level height change. It was replaced by key3DData in newer worlds
...
// Keys on a per-level basis. These are found only once in a leveldb level save.
const (
