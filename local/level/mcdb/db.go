...
// DB implements a level provider for the Minecraft level format, which
...
// passed using default options. If a level is present at the path, Open will
// parse its data and initialise the level with it. If the data cannot be
...
// Settings returns the level.Settings of the level loaded by the DB.
func (db *DB) Settings() *world.Settings {
	return db.set
}

// SaveSettings saves the level.Settings passed to the level.dat.
...
// LoadColumn reads a level.Column from the DB at a position and dimension in
...
func (db *DB) entitiesOld(k dbKey) ([]chunk.Entity, error) {
	...
	for buf.Len() != 0 {
		...
		if !ok {
			db.conf.Log.Error("missing unique LanID field, generating random", "data", fmt.Sprint(ent.Data))
			ent.ID = rand.Int64()
		}
		entities = append(entities, ent)
	}
...
// StoreColumn stores a level.Column at a position and dimension in the DB. An
...
// is not level.Overworld, the length of the index returned is 12. It is 8 otherwise.
