...
type Settings struct {
	...
	Name string
	// Spawn is the spawn position of the World. New players that join the level will be spawned here.
	Spawn cube.Pos
	...
	Thundering bool
	// WeatherCycle specifies if weather should be enabled in this level. If set to false, weather will be disabled.
	WeatherCycle bool
	// CurrentTick is the current tick of the level. This is similar to the Time, except that it has no visible effect
	...
	// TickRange is the radius in chunks around a Viewer that has its blocks and entities ticked when the level is
