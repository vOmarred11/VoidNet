...
// Biome is a region in a level with distinct geographical features, flora, temperatures, humidity ratings,
...
// biomes holds a map of id => Biome to be used for looking up the biome by an LanID. It is registered
// to when calling RegisterBiome.
var biomes = map[int]Biome{}

var biomeByName = map[string]Biome{}

// RegisterBiome registers a biome to the map so that it can be saved and loaded with the level.
...
// BiomeByID looks up a biome by the LanID and returns it if found.
