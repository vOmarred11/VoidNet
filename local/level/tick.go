...
func (t ticker) tick(tx *Tx) {
	...
	if s := w.set.Spawn; s[1] > tx.Range()[1] {
		...
		// the player should spawn at the highest position in the level.
		w.set.Spawn[1] = w.highestObstructingBlock(s[0], s[2]) + 1
	}
	if len(viewers) == 0 && w.set.CurrentTick != 0 {
		// Don't continue ticking if no viewers are in the level.
		w.set.Unlock()
		return
	}
	if w.advance {
...
// tickBlocksRandomly executes random block ticks in each sub chunk in the level that has at least one viewer
...
// tickEntities ticks all entities in the level, making sure they are still located in the correct chunks and
...
func (t ticker) tickEntities(tx *Tx, tick int64) {
	for handle, lastPos := range tx.World().entities {
		...
		if lastPos != chunkPos {
			...
			// When changing an entity's level, then teleporting it immediately, we could end up in a situation
