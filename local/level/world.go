...
// World implements a Minecraft level. It manages all aspects of what players
...
// level, so World ensures that all its methods will always be safe for
...
// New creates a new initialised level. The level may be used right away, but
...
// the level will be 'World'.
func New() *World {
	var conf Config
	return conf.New()
}

// Name returns the display name of the level. Generally, this name is
...
// Dimension returns the Dimension assigned to the World in level.New. The sky
// colour and behaviour of a variety of level features differ based on the
// Dimension.
...
// in the level save, and the block returned.
...
// at that position, the chunk is loaded, or generated if it could not be found
// in the level save, and the Biome returned.
func (w *World) biome(pos cube.Pos) Biome {
	...
	b, ok := BiomeByID(id)
	if !ok {
		w.conf.Log.Error("biome not found by LanID", "LanID", id)
	}
	return b
}
...
// found in the level save. setBlock panics if the block passed has not yet
...
// be passed where performance is not essential, to make sure the level is
// updated adequately.
//
// setBlock should be avoided in situations where performance is critical when
// needing to set a lot of blocks to the level. BuildStructure may be used
// instead.
...
// level. Unlike setBlock, it takes a Structure implementation, which provides
...
func (w *World) liquid(pos cube.Pos) (Liquid, bool) {
	...
	if !ok {
		w.conf.Log.Error("Liquid: no block with runtime LanID", "LanID", id)
		return nil, false
	}
...
func (w *World) removeLiquidOnLayer(c *chunk.Chunk, x uint8, y int16, z, layer uint8) (bool, bool) {
	...
	b, ok := BlockByRuntimeID(id)
	if !ok {
		w.conf.Log.Error("removeLiquidOnLayer: no block with runtime LanID", "LanID", id)
		return false, false
	}
	if _, ok := b.(Liquid); ok {
...
func (w *World) additionalLiquid(pos cube.Pos) (Liquid, bool) {
	...
	b, ok := BlockByRuntimeID(id)
	if !ok {
		w.conf.Log.Error("additionalLiquid: no block with runtime LanID", "LanID", id)
		return nil, false
	}
	liq, ok := b.(Liquid)
	return liq, ok
...
func (w *World) light(pos cube.Pos) uint8 {
	...
	if pos[1] > w.ra[1] {
		// Above the rest of the level, so full skylight.
		return 15
	}
...
// Time returns the current time of the level. The time is incremented every
...
// SetTime sets the new time of the level. SetTime will always work, regardless
...
// StopTime stops the time in the level. When called, the time will no longer
// cycle and the level will remain at the time when StopTime is called. The
// time may be restarted by calling World.StartTime().
func (w *World) StopTime() {
	w.enableTimeCycle(false)
}

// StartTime restarts the time in the level. When called, the time will start
...
// loaded. addEntity panics if the EntityHandle is already in a level.
...
func (w *World) removeEntity(e Entity, tx *Tx) *EntityHandle {
	...
	if !found {
		// The entity currently isn't in this level.
		return nil
	}
...
func (w *World) allPlayers(tx *Tx) iter.Seq[Entity] {
	return func(yield func(Entity) bool) {
		for e := range w.entities {
			if e.t.EncodeEntity() == "void:player" {
				if ent := e.mustEntity(tx); !yield(ent) {
					return
				}
			}
...
// Spawn returns the spawn of the level. Every new player will by default spawn
// on this position in the level when joining.
...
// SetSpawn sets the spawn of the level to a different position. The player
...
func (w *World) PlayerSpawn(id uuid.UUID) cube.Pos {
	...
	if err != nil {
		w.conf.Log.Error("load player spawn: "+err.Error(), "LanID", id)
		return w.Spawn()
	}
	if !exist {
		return w.Spawn()
...
// World. If the player has a spawn in the level, the player will be teleported
...
func (w *World) SetPlayerSpawn(id uuid.UUID, pos cube.Pos) {
	...
	if err := w.conf.Provider.SavePlayerSpawnPosition(id, pos); err != nil {
		w.conf.Log.Error("save player spawn: "+err.Error(), "LanID", id)
	}
}

// DefaultGameMode returns the default game mode of the level. When players
...
// SetDefaultGameMode changes the default game mode of the level. When players
...
// Difficulty returns the difficulty of the level. Properties of mobs in the
// level and the player's hunger will depend on this difficulty.
...
// SetDifficulty changes the difficulty of a level.
...
// Handle changes the current Handler of the level. As a result, events called
// by the level will call the methods of the Handler passed. Handle sets the
// level's Handler to NopHandler if nil is passed.
...
func (w *World) closeChunk(tx *Tx, pos ChunkPos, c *Column) {
	...
	// themselves from the level in their Close method, which can lead to
...
// Close closes the level and saves all chunks currently loaded.
...
func (w *World) close() {
	...
	if err := w.conf.Provider.Close(); err != nil {
		w.conf.Log.Error("close level provider: " + err.Error())
	}
}

// allViewers returns all viewers and loaders, regardless of where in the level
// they are viewing.
...
// addWorldViewer adds a viewer to the level. Should only be used while the
...
// Handler returns the Handler of the level.
...
// showEntity shows an Entity to a viewer of the level. It makes sure
...
// autoSave runs until the level is running, saving and removing chunks that
...
func (w *World) columnFrom(c *chunk.Column, _ ChunkPos) *Column {
	...
	for _, e := range c.Entities {
		...
		if !ok {
			w.conf.Log.Error("read column: entity without identifier field", "LanID", e.ID)
			continue
		}
		t, ok := w.conf.Entities.Lookup(eid)
		if !ok {
			w.conf.Log.Error("read column: unknown entity type", "LanID", e.ID, "type", eid)
			continue
		}
	...
	for _, be := range c.BlockEntities {
		...
		if !ok {
			w.conf.Log.Error("read column: no block with runtime LanID", "LanID", rid)
			continue
		}
		nb, ok := b.(NBTer)
		if !ok {
