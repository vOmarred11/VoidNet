...
// BucketFill is a sound played when a bucket is filled using a liquid source block from the level.
...
// BucketEmpty is a sound played when a bucket with a liquid in it is placed into the level.
type BucketEmpty struct {
	// Liquid is the liquid that the bucket places into the level.
	Liquid world.Liquid
