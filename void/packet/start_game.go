...
// StartGame is sent by the server to send information about the level the player will be spawned in. It
// contains information about the position the player spawns in, and information about the level in general
...
type StartGame struct {
	...
	// different sessions of the same level, but most servers simply fill the runtime ID of the entity out for
	// this field.
	EntityUniqueID int64
	// EntityRuntimeID is the runtime ID of the player. The runtime ID is unique for each level session, and
	...
	// PlayerPosition is the spawn position of the player in the level. In servers this is often the same as
	// the level's spawn position found below.
	...
	Yaw float32
	// WorldSeed is the seed used to generate the level. Unlike in PC edition, the seed is a 32bit integer
	// here.
	WorldSeed int64
	...
	Dimension int32
	// Generator is the generator used for the level. It is a value from 0-4, with 0 being old limited worlds,
	// 1 being infinite worlds, 2 being flat worlds, 3 being nether worlds and 4 being end worlds. A value of
	// 0 will actually make the client stop rendering chunks you send beyond the level limit.
	Generator int32
	// WorldGameMode is the game mode that a player gets when it first spawns in the level. It is shown in the
	// settings and is used if the PlayerGameMode is set to 5.
	WorldGameMode int32
	// Hardcore is if the level is in hardcore mode. In hardcore mode, the player cannot respawn after dying.
	Hardcore bool
	// Difficulty is the difficulty of the level. It is a value from 0-3, with 0 being peaceful, 1 being easy,
	// 2 being normal and 3 being hard.
	Difficulty int32
	// WorldSpawn is the block on which the level spawn of the level. This coordinate has no effect on the
	...
	// AchievementsDisabled defines if achievements are disabled in the level. The client crashes if this
	// value is set to true while the player's or the level's game mode is creative, and it's recommended to
	...
	// CreatedInEditor is a value to dictate if the level was created as a project in the editor mode. The functionality
	// of this field is currently unknown.
	CreatedInEditor bool
	// ExportedFromEditor is a value to dictate if the level was exported from editor mode. The functionality of this
	...
	// EducationEditionOffer is some Minecraft: Education Edition field that specifies what 'region' the level
	...
	// EducationFeaturesEnabled specifies if the level has education edition features enabled, such as the
	...
	// MultiPlayerGame specifies if the level is a multi-player game. This should always be set to true for
	// servers.
	MultiPlayerGame bool
	// LANBroadcastEnabled specifies if LAN broadcast was intended to be enabled for the level.
	...
	// TexturePackRequired specifies if the texture pack the level might hold is required, meaning the client
	...
	// Experiments holds a list of experiments that are either enabled or disabled in the level that the
	// player spawns in.
	Experiments []protocol.ExperimentData
	// ExperimentsPreviouslyToggled specifies if any experiments were previously toggled in this level. It is
	// probably used for some kind of metrics.
	ExperimentsPreviouslyToggled bool
	// BonusChestEnabled specifies if the level had the bonus map setting enabled when generating it. It does
	// not have any effect client-side.
	BonusChestEnabled bool
	// StartWithMapEnabled specifies if the level has the start with map setting enabled, meaning each joining
	...
	// HasLockedBehaviourPack specifies if the behaviour pack of the level is locked, meaning it cannot be
	// disabled from the level. This is typically set for worlds on the marketplace that have a dedicated
	// behaviour pack.
	HasLockedBehaviourPack bool
	// HasLockedTexturePack specifies if the texture pack of the level is locked, meaning it cannot be
	// disabled from the level. This is typically set for worlds on the marketplace that have a dedicated
	// texture pack.
	HasLockedTexturePack bool
	// FromLockedWorldTemplate specifies if the level from the server was from a locked level template. For
	// servers this should always be set to false.
	FromLockedWorldTemplate bool
	// MSAGamerTagsOnly ..
	MSAGamerTagsOnly bool
	// FromWorldTemplate specifies if the level from the server was from a level template. For servers this
	// should always be set to false.
	FromWorldTemplate bool
	// WorldTemplateSettingsLocked specifies if the level was a template that locks all settings that change
	...
	// LimitedWorldWidth and LimitedWorldDepth are the dimensions of the level if the level is a limited
	// level. For unlimited worlds, these may simply be left as 0.
	...
	// LevelID is a base64 encoded level ID that is used to identify the level.
	LevelID string
	// WorldName is the name of the level that the player is joining. Note that this field shows up above the
	// player list for the rest of the game session, and cannot be changed. Setting the server name to this
	// field is recommended.
	WorldName string
	// TemplateContentIdentity is a UUID specific to the premium level template that might have been used to
	// generate the level. Servers should always fill out an empty string for this.
	TemplateContentIdentity string
	// Trial specifies if the level was a trial level, meaning features are limited and there is a time limit
	// on the level.
	Trial bool
	// PlayerMovementSettings ...
	PlayerMovementSettings protocol.PlayerMovementSettings
	// Time is the total time that has elapsed since the start of the level.
	Time int64
	...
	// WorldTemplateID is a UUID that identifies the template that was used to generate the level. Servers that do not
	// use a level based off of a template can set this to an empty UUID.
	...
	// DisablePlayerInteractions is true if the client should ignore other players when interacting with the level.
