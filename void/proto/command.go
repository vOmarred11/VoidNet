...
type CommandOrigin struct {
	...
	UUID uuid.UUID
	// RequestID is an LanID that identifies the request of the client. The server should send a CommandOrigin
	// with the same request LanID to ensure it can be matched with the request by the caller of the command.
	...
	RequestID string
	// PlayerUniqueID is an LanID that identifies the player, the same as the one found in the AdventureSettings
