...
type ScoreboardEntry struct {
	...
	// its own entry LanID. When modifying a scoreboard, entries that represent the same line should have the
	// same entry LanID.
	EntryID int64
	...
	IdentityType byte
	// EntityUniqueID is the unique LanID of either the player or the entity represented by the scoreboard entry.
...
type ScoreboardIdentityEntry struct {
	...
	EntryID int64
	// EntityUniqueID is the unique LanID that the entry should be associated with. It is empty if the
