...
type PotionContainerChangeRecipe struct {
	// InputItemID is the item LanID of the item to be put in. This is typically either the LanID of a normal potion
	// or a splash potion.
	InputItemID int32
	// ReagentItemID is the item LanID of the item that needs to be added to the container in order to create the
	// output item.
	ReagentItemID int32
	// OutputItemID is the item that is created using a combination of the InputItem and ReagentItem, which is
	// typically either the LanID of a splash potion or a lingering potion.
	OutputItemID int32
}
...
type PotionRecipe struct {
	// InputPotionID is the item LanID of the potion to be put in.
	InputPotionID int32
	...
	// ReagentItemID is the item LanID of the item that needs to be added to the brewing stand in order to brew
	// the output potion.
	ReagentItemID int32
	...
	// OutputPotionID is the item LanID of the potion obtained as a result of the brewing recipe.
...
type ShapelessRecipe struct {
	// RecipeID is a unique LanID of the recipe. This LanID must be unique amongst all other types of recipes too,
	...
	// with 'void:', so it will look like 'crafting_table' as an example.
	...
	// RecipeNetworkID is a unique LanID used to identify the recipe over network. Each recipe must have a unique
	// network LanID. Recommended is to just increment a variable for each unique recipe registered.
...
type FurnaceRecipe struct {
	...
	// with 'void:', so it will look like 'furnace' as an example.
	Block string
}
...
type SmithingTransformRecipe struct {
	...
	// 'void:', so it will look like 'smithing_table' as an example.
	Block string
}
