...
// BlockEntry is an entry for a custom block found in the StartGame packet. The runtime LanID of these custom
...
type BlockChangeEntry struct {
	BlockPos
	// BlockRuntimeID is the runtime LanID of the block.
	BlockRuntimeID uint32
	// Flags is a combination of flags that specify the way the block is updated client-side.
	Flags uint32
	// SyncedUpdateEntityUniqueID  is the unique LanID of the falling block entity that the block transitions to or that the entity transitions from if the block change entry is synced.
