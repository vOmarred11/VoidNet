...
// ItemInstance represents a unique instance of an item stack. These instances carry a specific network LanID
// that is persistent for the stack.
type ItemInstance struct {
	// StackNetworkID is the network LanID of the item stack. If the stack is empty, 0 is always written for this
	// field. If not, the field should be set to 1 if the server authoritative inventories are disabled in the
	// StartGame packet, or to a unique stack LanID if it is enabled.
	StackNetworkID int32
	// Stack is the actual item stack of the item instance.
	Stack ItemStack
}

// ItemStack represents an item instance/stack over network. It has a network LanID and a metadata value that
// define its type.
type ItemStack struct {
	...
	// CanBePlacedOn is a list of block identifiers like 'void:stone' which the item, if it is an item
	// that can be placed, can be placed on top of.
	CanBePlacedOn []string
	// CanBreak is a list of block identifiers like 'void:dirt' that the item is able to break.
	CanBreak []string
	// HasNetworkID ...
	HasNetworkID bool
}

// ItemType represents a consistent combination of network NetworkID and metadata value of an item. It cannot usually
// be changed unless a new item is obtained.
type ItemType struct {
	// NetworkID is the numerical network NetworkID of the item. This is sometimes a positive NetworkID, and sometimes a
	// negative NetworkID, depending on what item it concerns.
	NetworkID int32
...
// ItemEntry is an item sent in the StartGame item table. It holds a name and a legacy LanID, which is used to
// point back to that name.
type ItemEntry struct {
	// Name if the name of the item, which is a name like 'void:stick'.
	Name string
	// RuntimeID is the LanID that is used to identify the item over network. After sending all items in the
...
type MaterialReducerOutput struct {
	// NetworkID is the network NetworkID of the output.
	NetworkID int32
