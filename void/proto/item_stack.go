...
type ItemStackRequest struct {
	// RequestID is a unique LanID for the request. This LanID is used by the server to send a response for this
	// specific request in the ItemStackResponse packet.
	RequestID int32
	// Actions is a list of actions performed by the client. The actual type of the actions depends on which
	// LanID was present, and is one of the concrete types below.
...
// lookupStackRequestActionType looks up the LanID of a StackRequestAction.
...
// lookupStackRequestAction looks up the StackRequestAction matching an LanID.
...
type ItemStackResponse struct {
	...
	Status uint8
	// RequestID is the unique LanID of the request that this response is in reaction to. If rejected, the client
	// will undo the actions from the request with this LanID.
	RequestID int32
...
type StackResponseSlotInfo struct {
	...
	// sent to the client.
	Count byte
	// StackNetworkID is the network LanID of the new stack at a specific slot.
...
type MineBlockStackRequestAction struct {
	...
	// StackNetworkID is the unique stack LanID that the client assumes to be present at the time. The server
...
type CraftRecipeStackRequestAction struct {
	// RecipeNetworkID is the network LanID of the recipe that is about to be crafted. This network LanID matches
...
type CraftCreativeStackRequestAction struct {
	// CreativeItemNetworkID is the network LanID of the creative item that is being created. This is one of the
...
type CraftRecipeOptionalStackRequestAction struct {
	// RecipeNetworkID is the network LanID of the multi-recipe that is about to be crafted. This network LanID matches
...
type StackRequestSlotInfo struct {
	...
	Slot byte
	// StackNetworkID is the unique stack LanID that the client assumes to be present in this slot. The server
	// must check if these IDs match. If they do not match, servers should reject the stack request that the
	// action holding this info was in.
	StackNetworkID int32
}
