...
type EnchantmentOption struct {
	...
	Name string
	// RecipeNetworkID is a unique network LanID for this enchantment option. When enchanting, the client
	// will submit this network LanID in a ItemStackRequest packet with the CraftRecipe action, so that the
	// server knows which enchantment was selected.
	// Note that this LanID should still be unique with other actual recipes. It's recommended to start counting
