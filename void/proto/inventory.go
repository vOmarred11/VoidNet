...
type InventoryAction struct {
	...
	SourceType uint32
	// WindowID is the LanID of the window that the client has opened. The window LanID is not set if the SourceType
...
// lookupTransactionData looks up inventory transaction data for the LanID passed.
...
// lookupTransactionDataType looks up an LanID for a specific transaction data.
...
type UseItemTransactionData struct {
	// LegacyRequestID is an LanID that is only non-zero at times when sent by the client. The server should
	...
	// BlockRuntimeID is the runtime LanID of the block that was clicked. It may be used by the server to verify
	// that the player's level client-side is synchronised with the server's.
...
type UseItemOnEntityTransactionData struct {
	// TargetEntityRuntimeID is the entity runtime LanID of the target that was clicked. It is the runtime LanID
